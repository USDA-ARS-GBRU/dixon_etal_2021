...
ggplot(aes(x=week, y=count, group=treatment_type, color=treatment_type), data=data_filtered3,) +
  ...
  ylab("Mosquito count") +
  xlab("Week")
ggsave("albopictus_bg_weekly.pdf")
```
We will first focus on BG traps and female _Aedes aegypti_ mosquitoes in BG traps
...
```
There is not a significant effect of treatment on counts of female _Aedes albopictus_ in SAGO traps
...
```
There is not a significant effect of treatment on counts on the full Mosquito population in SAGO traps
